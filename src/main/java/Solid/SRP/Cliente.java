...
public class Cliente {
    ...
    private static void showEmpDetail(Empleado emp) {
        ...
        //Genera el ID
        GeneradorIDEmpleado idGenerado = new GeneradorIDEmpleado();
        String id = idGenerado.generateEmpId(emp.firstName);
        emp.empId = id;
        System.out.println("El ID del empleado es: "+ emp.empId);

        //Verifica el nivel laboral
        SeniorityChecker checker = new SeniorityChecker();
        System.out.println("Este empleado es un" + " empleado " + checker.checkSeniority(emp.experienceInYears));
    }
}
